...
def plot_temp_daily(file, llat, llon, fecha, carpeta):
    """
    Estimates daily precipitation for each day since start till seven days later
    and plot it.
    """
    ...
    for t in np.arange(0, np.shape(TMAX)[0]):
        ...
        z2 = TMIN[t, :, :] - 273.
        z2[lndsfc == 0.] = np.nan
        #print(np.nanmin(z2), np.nanmax(z2))
        # Tmax
        CS1 = ax1.contourf(x, y, z1, levels=bounds, cmap=cMap, norm=norm,
                           transform=ccrs.PlateCarree())
        cbaxes = inset_axes(ax1, width="3%", height="50%", loc=4)
        cb = fig1.colorbar(CS1, cax=cbaxes, drawedges=True, ticks=bounds)
        cb.ax.yaxis.set_ticks_position('left')
        for label in cb.ax.yaxis.get_ticklabels()[::2]:
            label.set_visible(False)
        fig_name = carpeta + 'TMAX_' + fdates[t].strftime('%Y%m%d') + '.png'
        ...
        for label in cb1.ax.yaxis.get_ticklabels()[::2]:
            label.set_visible(False)
        fig_name = carpeta + 'TMIN_' + fdates[t].strftime('%Y%m%d') + '.png'
        fig2.savefig(fig_name, dpi=200)


def plot_wind_daily(file, llat, llon, fecha, carpeta):
    """
    """
    ...
    for t in np.arange(0, np.shape(u)[0]):
        ...
        cb.ax.yaxis.set_ticks_position('left')
        fig_name = carpeta + 'SurfWind_' + fdates[t].strftime('%Y%m%d%H') + '.png'
...
if __name__ == '__main__':
    ...
    plot_temp_daily(file, l_lat, l_lon, mydate, ofolder)
    plot_wind_daily(file, l_lat, l_lon, mydate, ofolder)
    file.close()
